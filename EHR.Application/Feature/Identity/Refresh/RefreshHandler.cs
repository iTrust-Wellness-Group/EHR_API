...
{
    public class RefreshHandler : BaseHandler, IRequestHandler<RefreshReq, ResponseData<RefreshRes>>
    {

        IServiceContext service;
        public RefreshHandler(IServiceContext service)
        {
            this.service = service;
        }
        public async Task<ResponseData<RefreshRes>> Handle(RefreshReq request, CancellationToken cancellationToken)
        {
            ...
            if (isVaild)
            {
                ...
                //});
                var data = new ResponseData<RefreshRes>(response);
                return await Task.FromResult(data);
            }
            else
            {
                var data = new ResponseData<RefreshRes>(401, "Invaild RefreshToken");
                return await Task.FromResult(data);
            }
        }
    }
